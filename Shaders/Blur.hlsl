...
void HorizontalBlurCS(int3 groupThreadID : SV_GroupThreadID, //组内线程ID
...
int3 dispatchThreadID : SV_DispatchThreadID)//分派ID
{
    float weights[11] = { w0, w1, w2, w3, w4, w5, w6, w7, w8, w9, w10 }; //权重数组
   //对图像上侧的越界采样进行钳位操作
    if (groupThreadID.y < gBlurRadius)//如果是最上侧5个像素
    {
        //钳位左边的模糊半径个（5个）像素,y都是0
    ...
    {
        //钳位右边的模糊半径个（5个）像素,y都是gInput.Length.y - 1,即最后一个像素索引
