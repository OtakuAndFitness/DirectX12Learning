...
struct SkinnedModelInstance {
	SkinnedData* skinedInfo = nullptr;
	...
	void UpdateSkinnedAnimation(float dt) {
		timePos += dt;
		if (timePos > skinedInfo->GetClipEndTime(clipName)) {
			timePos = 0.0f;
		}
		skinedInfo->GetFinalTransforms(clipName, timePos, finalTransforms);
	}
};
...
class SkinnedMeshApp : public D3D12App {
public:
...
private:
	...
	vector<D3D12_INPUT_ELEMENT_DESC> mSkinnedInpuLayout;
	...
	vector<string> mSkinnedTextureNames;
...
array<const CD3DX12_STATIC_SAMPLER_DESC, 7> SkinnedMeshApp::GetStaticSamplers()
{
...
SkinnedMeshApp::SkinnedMeshApp(HINSTANCE hInstance) : D3D12App(hInstance)
{
...
void SkinnedMeshApp::Draw()
{
	...
	cmdList->SetGraphicsRootShaderResourceView(3, //根参数的起始索引
		matBuffer->GetGPUVirtualAddress());

	cmdList->SetGraphicsRootDescriptorTable(4, mNullSrv);

	cmdList->SetGraphicsRootDescriptorTable(5, mSrvHeap->GetGPUDescriptorHandleForHeapStart());
	...
	mSsaoMap->ComputeSsao(cmdList.Get(), mCurrFrameResource, 2);
...
void SkinnedMeshApp::OnKeyboardInput()
{
...
void SkinnedMeshApp::UpdateObjectCBs()
{
...
void SkinnedMeshApp::UpdateSkinnedCBs()
{
	...
	copy(begin(mSkinnedModelInst->finalTransforms), end(mSkinnedModelInst->finalTransforms), &skinnedConstants.boneTransforms[0]);

	currSkinnedCB->CopyData(0, skinnedConstants);
}

void SkinnedMeshApp::UpdateMaterialBuffer()
{
...
void SkinnedMeshApp::UpdateMainPassCB()
{
...
void SkinnedMeshApp::UpdateShadowTransform()
{
	// 主光才投射物体阴影
...
void SkinnedMeshApp::UpdateShadowPassCB()
{
...
void SkinnedMeshApp::UpdateSsao()
{
...
void SkinnedMeshApp::LoadTextures()
{
	...
	{
		string diffuseName = mSkinnedMats[i].DiffuseMapName;
		string normalName = mSkinnedMats[i].NormalMapName;

		wstring diffuseFileName = L"Textures/" + AnsiToWString(diffuseName);
		wstring normalFileName = L"Textures/" + AnsiToWString(normalName);
	...
	{
		if (mTextures.find(texNames[i]) == end(mTextures)) {
...
void SkinnedMeshApp::BuildDescriptorHeaps()
{
	...
	mSsaoMap->BuildDescriptors(mDepthStencilBuffer.Get(), 
...
void SkinnedMeshApp::BuildShadersAndInputLayout()
{
	...
	mShaders["skinnedDrawNormalVS"] = d3dUtil::CompileShader(L"Shaders\\DrawNormal.hlsl", skinnedDefines, "VS", "vs_5_1");
...
void SkinnedMeshApp::BuildGeometry()
{
...
void SkinnedMeshApp::LoadSkinnedModel()
{
	...
	mSkinnedModelInst->skinedInfo = &mSkinnedInfo;
...
void SkinnedMeshApp::BuildPSO()
{
	...
	D3D12_GRAPHICS_PIPELINE_STATE_DESC skinnedOpaqueDesc = opaqueDesc;
	skinnedOpaqueDesc.InputLayout = { mSkinnedInpuLayout.data(), (UINT)mSkinnedInpuLayout.size() };
	skinnedOpaqueDesc.VS = {
		reinterpret_cast<BYTE*>(mShaders["skinnedVS"]->GetBufferPointer()),
		mShaders["skinnedVS"]->GetBufferSize()
	};
	skinnedOpaqueDesc.PS = {
		reinterpret_cast<BYTE*>(mShaders["standardPS"]->GetBufferPointer()),
		mShaders["standardPS"]->GetBufferSize()
	};
	ThrowIfFailed(d3dDevice->CreateGraphicsPipelineState(&skinnedOpaqueDesc, IID_PPV_ARGS(&mPSOs["skinnedOpaque"])));


	D3D12_GRAPHICS_PIPELINE_STATE_DESC shadowMapDesc = opaqueDesc;
	...
	D3D12_GRAPHICS_PIPELINE_STATE_DESC skinnedShadowMapDesc = opaqueDesc;
	skinnedShadowMapDesc.InputLayout = { mSkinnedInpuLayout.data(), (UINT)mSkinnedInpuLayout.size() };
	skinnedShadowMapDesc.VS = {
		reinterpret_cast<BYTE*>(mShaders["skinnedShadowVS"]->GetBufferPointer()),
		mShaders["skinnedShadowVS"]->GetBufferSize()
	};
	skinnedShadowMapDesc.PS = {
		reinterpret_cast<BYTE*>(mShaders["shadowOpaquePS"]->GetBufferPointer()),
		mShaders["shadowOpaquePS"]->GetBufferSize()
	};
	skinnedShadowMapDesc.RTVFormats[0] = DXGI_FORMAT_UNKNOWN;
	skinnedShadowMapDesc.NumRenderTargets = 0;
	ThrowIfFailed(d3dDevice->CreateGraphicsPipelineState(&skinnedShadowMapDesc, IID_PPV_ARGS(&mPSOs["skinnedShadow_opaque"])));

	D3D12_GRAPHICS_PIPELINE_STATE_DESC debugDesc = opaqueDesc;
	...
	D3D12_GRAPHICS_PIPELINE_STATE_DESC drawNormalDesc = opaqueDesc;
	drawNormalDesc.VS = {
	...
	D3D12_GRAPHICS_PIPELINE_STATE_DESC skinnedDrawNormalDesc = drawNormalDesc;
	skinnedDrawNormalDesc.InputLayout = { mSkinnedInpuLayout.data(), (UINT)mSkinnedInpuLayout.size() };
	skinnedDrawNormalDesc.VS = {
		reinterpret_cast<BYTE*>(mShaders["skinnedDrawNormalVS"]->GetBufferPointer()),
		mShaders["skinnedDrawNormalVS"]->GetBufferSize()
	};
	skinnedDrawNormalDesc.PS = {
		reinterpret_cast<BYTE*>(mShaders["drawNormalPS"]->GetBufferPointer()),
		mShaders["drawNormalPS"]->GetBufferSize()
	};
	ThrowIfFailed(d3dDevice->CreateGraphicsPipelineState(&skinnedDrawNormalDesc, IID_PPV_ARGS(&mPSOs["skinnedDrawNormal"])));

	D3D12_GRAPHICS_PIPELINE_STATE_DESC ssaoDesc = opaqueDesc;
...
void SkinnedMeshApp::BuildFrameResource()
{
...
void SkinnedMeshApp::BuildMaterials()
{
	...
	for (UINT i = 0; i < mSkinnedMats.size(); i++)
	{
		...
		mMaterials[mat->name] = move(mat);
	}
}

void SkinnedMeshApp::BuildRenderItem()
{
	...
	for (UINT i = 0; i < mSkinnedMats.size(); i++)
	{
		...
		renderItem->objCBIndex = followObjCBIndex++;
		...
		mRitemLayer[(int)RenderLayer::SkinnedOpaque].push_back(renderItem.get());
		mAllRenderItems.push_back(move(renderItem));
	}
}

void SkinnedMeshApp::DrawRenderItems(vector<RenderItem*>& ritems)
{
...
void SkinnedMeshApp::DrawNormalsAndDepth()
{
	...
	cmdList->SetPipelineState(mPSOs["skinnedDrawNormal"].Get());
	DrawRenderItems(mRitemLayer[(int)RenderLayer::SkinnedOpaque]);
