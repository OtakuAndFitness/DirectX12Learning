...
public:
	...
	bool LoadM3d(const string& fileName, vector<Vertex>& vertices, vector<USHORT>& indices, vector<Subset>& subsets, vector<M3dMaterial>& mats);

	bool LoadM3d(const string& fileName, vector<SkinnedVertex>& vertices, vector<USHORT>& indices, vector<Subset>& subsets, vector<M3dMaterial>& mats, SkinnedData& skinInfo);
